...
void main(void)
{
    ...
    if ( Radius > 0.0 ) {
        ...
        for ( int i = 1; 3 > i; i++ ) {
            vec4 valueRight = weight[i] * texture2D(MainTexture, vec2( (gl_FragCoord.x + offset[i] )/ Width,(gl_FragCoord.y  ) / Height ) );
            vec4 valueLeft = weight[i] * texture2D(MainTexture, vec2( (gl_FragCoord.x - offset[i] ) / Width, (gl_FragCoord.y  ) / Height ) );
