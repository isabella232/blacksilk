...
function create_helper {
	...
	echo -ne '\r\n' >> "$BUILD_HELPER_DEBUG"
	echo -ne 'msbuild VisualStaticMTD.sln /p:configuration=debug /p:platform=x64 /p:PlatformToolset=v142 /p:PreferredToolArchitecture=x64\r\n' >> "$BUILD_HELPER_DEBUG"
    chmod +x "$BUILD_HELPER_DEBUG"

	echo -ne "@echo off\r\n\r\ncall \"$VSNEWCOMNTOOLS\\\\vcvars64.bat\"\r\n" > "$BUILD_HELPER_RELEASE"
	echo -ne '\r\n' >> "$BUILD_HELPER_RELEASE"
	echo -ne 'msbuild VisualStaticMTD.sln /p:configuration=release /p:platform=x64 /p:PlatformToolset=v142 /p:PreferredToolArchitecture=x64\r\n' >> "$BUILD_HELPER_RELEASE"
...
function doCopy {
    ...
    cp -r "$BUILD_DIR/VisualMagick/lib_debug"/* "$TARGET_DIR/bin/debug"
    cp -r "$BUILD_DIR/VisualMagick/lib_release"/* "$TARGET_DIR/bin/release"
